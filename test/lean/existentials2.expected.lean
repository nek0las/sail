...
structure st where
  One : (Int × Int)
  Two : Sigma (fun k_n => (Int × (BitVec k_n)))
  Three : (Int × Sigma (fun k_n => (Int × (BitVec k_n))))
  deriving BEq, Inhabited, Repr

inductive ast2 where
  | Foo2 (_ : Sigma (fun k_n => (Int × (Int × (BitVec k_n)))))
  deriving Inhabited, BEq, Repr
  open ast2
...
/-- Type quantifiers: k_ex1223_ : Bool, k_ex1222_ : Bool -/
...
def test_struct1 (i : Int) (v : (BitVec 16)) : st :=
  { One := (i, i)
    Two := (16, v)
    Three := (i, (16, v)) }

/-- Type quantifiers: i : Int -/
def test_struct2 (i : Int) (v : (BitVec 16)) (st0 : st) : st :=
  let st1 := st0
  let st1 : st := { st1 with Two := (16, v) }
  { st1 with Three := (i, (16, v)) }

def test11 (x : ast2) : Int :=
  match x with
  | .Foo2 (y, z) =>
    (match z with
    | (_, v) => (BitVec.toNatInt v))

/-- Type quantifiers: i : Int -/
def test12 (i : Int) (v : (BitVec 16)) : ast2 :=
  (Foo2 (i, (16, v)))
