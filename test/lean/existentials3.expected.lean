...
/-- Type quantifiers: k_ex992_ : Bool, k_ex991_ : Bool -/
...
/-- Type quantifiers: x : Int -/
def f (x : Int) : Sigma (fun k_n => (Int × (BitVec k_n))) :=
  if ((x >b 0) : Bool)
  then (x, (sail_ones x))
  else (1, 0#1)

def use_f (_ : Unit) : SailM Unit := do
  let x := 5
  let (xx, xv) := (f x)
  assert (xx == 5) "existentials3.sail:23.16-23.17"
  assert (xv == 0b11111#5) "existentials3.sail:24.22-24.23"
  let y := (Neg.neg 5)
  let (yy, yv) := (f y)
  assert (yy == 1) "existentials3.sail:27.16-27.17"
  assert (yv == 0#1) "existentials3.sail:28.18-28.19"

/-- Type quantifiers: n : Nat, n > 0 -/
def g (n : Nat) : (BitVec n) :=
  (sail_ones n)

def g_size (_ : Unit) : Sigma (fun k_n => Int) :=
  8

def use_g (_ : Unit) : SailM Unit := do
  assert (((BitVec.toNatInt (g (g_size ()))) : Int) == 255) "existentials3.sail:51.43-51.44"
